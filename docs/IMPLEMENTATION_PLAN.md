...
- 선행 task: `T-030-data-model-audit`, `T-100-share-target-flow`, `T-120-media-store-batch-query`
- Blocked by: `T-120` 미완료
...
- 선행 task: `T-030-data-model-audit`, `T-120-media-store-batch-query`
- Blocked by: `T-120` 미완료
...
- 선행 task: `T-100-share-target-flow`, `T-110-quick-tile-flow`, `T-120-media-store-batch-query`
- Blocked by: `T-120` 미완료
- Ready criteria: Share/Tile/MediaStore 저장 결과 이벤트가 View/UI에 전달됨
- 병렬 진행 가능 여부: 예
...
현재 바로 시작 가능한 task는 `T-120-media-store-batch-query` 하나입니다.

`T-010-agents-and-docs-setup`, `T-000-current-code-audit`, `T-020-architecture-baseline`, `T-030-data-model-audit`, `T-050-permission-and-manifest-baseline`, `T-040-navigation-baseline`, `T-100-share-target-flow`, `T-110-quick-tile-flow`는 완료된 상태입니다. 이제 앱 실행 시 Last Sync Time 이후 새 이미지를 MediaStore에서 batch query하는 흐름을 구현합니다.

## 아직 시작하면 안 되는 작업 예시

- `T-130-storage-access-framework-picker`: 순차 진행 정책상 `T-120` 완료 후 Ready 전환
- `T-140-enrichment-ocr-og-pipeline`: `T-120`의 이미지 수집 경로가 완료된 뒤 시작 가능
...
1. `T-120-media-store-batch-query`
2. 완료 후 `T-130-storage-access-framework-picker`를 Ready 전환합니다.
